...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    std::string createFileTestPath = "test.txt";
    bool createFileResult = fm.createFile(createFileTestPath);
    if (createFileResult) {
        MessageBoxA(NULL, "Test file created", "Success", MB_OK);
    } else {
        MessageBoxA(NULL, "Test file creation failed", "Error", MB_OK);
    }

    // Test fileExists function
    bool fileExistsResult = fm.fileExists(createFileTestPath);
    if (fileExistsResult) {
        MessageBoxA(NULL, "Test file exists", "Success", MB_OK);
    } else {
        MessageBoxA(NULL, "Test file does not exist", "Error", MB_OK);
    }

    // Test readLines function
    std::vector<std::string> readLinesResult = fm.readLines(createFileTestPath);
    if (!readLinesResult.empty()) {
        std::string readLinesText = "";
        for (auto& line : readLinesResult) {
            readLinesText += line + "\n";
        }
        MessageBoxA(NULL, readLinesText.c_str(), "File contents", MB_OK);
    } else {
        MessageBoxA(NULL, "Test file is empty", "Error", MB_OK);
    }

    // Test writeLines function
    std::vector<std::string> writeLinesTestData = {"Test line 1", "Test line 2", "Test line 3"};
    bool writeLinesResult = fm.writeLines(createFileTestPath, writeLinesTestData);
    if (writeLinesResult) {
        MessageBoxA(NULL, "Test lines written to file", "Success", MB_OK);
    } else {
        MessageBoxA(NULL, "Test lines could not be written to file", "Error", MB_OK);
    }

    // Test copyFile function
    std::string copyFilePath = "copy.txt";
    bool copyFileResult = fm.copyFile(createFileTestPath, copyFilePath);
    if (copyFileResult) {
        MessageBoxA(NULL, "Test file copied", "Success", MB_OK);
    } else {
        MessageBoxA(NULL, "Test file could not be copied", "Error", MB_OK);
    }

    // Test deleteFile function
    bool deleteFileResult = fm.deleteFile(createFileTestPath);
    if (deleteFileResult) {
        MessageBoxA(NULL, "Test file deleted", "Success", MB_OK);
    } else {
        MessageBoxA(NULL, "Test file could not be deleted", "Error", MB_OK);
    }
